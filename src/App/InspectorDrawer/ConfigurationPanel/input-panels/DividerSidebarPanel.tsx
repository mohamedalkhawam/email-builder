...
import { type DividerProps, DividerPropsDefaults, DividerPropsSchema } from 'components/block-divider';
...
export default function DividerSidebarPanel({ data, setData }: DividerSidebarPanelProps) {
  ...
  return (
    <BaseSidebarPanel title='Divider block'>
      <ColorInput label='Color' defaultValue={lineColor} onChange={(lineColor) => updateData({ ...data, props: { ...data.props, lineColor } })} />
      <SliderInput
        label='Height'
      ...
      />
      <MultiStylePropertyPanel names={['backgroundColor', 'padding']} value={data.style} onChange={(style) => updateData({ ...data, style })} />
    </BaseSidebarPanel>
  );
}
